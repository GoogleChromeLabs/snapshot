'use strict';class ViewState{}var constants={IMAGE_TYPE:'image/jpeg',SUPPORTS_IMAGE_CAPTURE:'ImageCapture'in window,SUPPORTS_MEDIA_DEVICES:'mediaDevices'in navigator};const random=(a,b)=>{let c=Math.random();return c*=b-a,c+=a,c};class FilterTransform{static from(a){const b=new FilterTransform;return a&&(b.saturation=a.saturation||b.saturation,b.warmth=a.warmth||b.warmth,b.sharpen=a.sharpen||b.sharpen,b.blur=a.blur||b.blur,b.brightness=a.brightness||b.brightness,b.contrast=a.contrast||b.contrast,b.grey=a.grey||b.grey,b.vignette=a.vignette||b.vignette),b}constructor(){this.saturation=1,this.warmth=0,this.sharpen=0,this.blur=1,this.brightness=1,this.contrast=1,this.grey=0.5,this.vignette=2}randomize(){this.saturation=random(0,2),this.warmth=random(-0.08,0.08),this.sharpen=random(-2,2),this.blur=random(0.01,4),this.brightness=random(0,2),this.contrast=random(0,2),this.grey=random(0,1),this.vignette=random(0.2,2)}}function dataUrlToArrayBuffer(a){const b=atob(a.split(',')[1]),c=new Uint8Array(b.length);for(let d=0;d<b.length;d++)c[d]=b.charCodeAt(d);return c.buffer}async function canvasToBlob(a,b){if(a.toBlob){const c=new Promise((c)=>{a.toBlob((a)=>c(a),b)});return c}const c=a.toDataURL(b),d=dataUrlToArrayBuffer(c);return new Blob([d],{type:b})}function blobToArrayBuffer(a){return new Promise((b,c)=>{const d=new FileReader;d.addEventListener('loadend',async()=>{b(d.result)}),d.addEventListener('error',c),d.readAsArrayBuffer(a)})}const DB_VERSION=3;class ImageDB{constructor(){const a=indexedDB.open('image-db',DB_VERSION);this.dbPromise=new Promise((b,c)=>{this.dbResolve=b,this.dbReject=c,a.onerror=(a)=>this.dbReject(a),a.onupgradeneeded=(a)=>this.createObjectStore(a),a.onsuccess=()=>this.dbOpened(a)})}storeRecord(a){null===a.id&&delete a.id;const b=new Promise((b,c)=>{this.dbPromise.then((d)=>{const e=d.transaction(['list'],'readwrite'),f=e.objectStore('list').put(a);f.onsuccess=()=>b(f.result),f.onerror=c}).catch(c)});return b}retrieveRecord(a){const b=new Promise((b,c)=>{this.dbPromise.then((d)=>{const e=d.transaction(['list'],'readonly'),f=e.objectStore('list').get(a);f.onsuccess=()=>b(f.result),f.onerror=c}).catch(c)});return b}async storeMedia(a,b){const c=await blobToArrayBuffer(a),d={media:c,type:a.type},e=new Promise((a,c)=>{this.dbPromise.then((e)=>{const f=e.transaction(['media'],'readwrite'),g=f.objectStore('media').put(d,b);g.onsuccess=()=>a(g.result),g.onerror=c}).catch(c)});return e}retrieveMedia(a){const b=new Promise((b,c)=>{this.dbPromise.then((d)=>{const e=d.transaction(['media'],'readonly'),f=e.objectStore('media').get(a);f.onsuccess=()=>{const a=f.result,c=new Blob([a.media],{type:a.type});b(c)},f.onerror=c}).catch(c)});return b}all(){const a=new Promise((a,b)=>{this.dbPromise.then((c)=>{const d=c.transaction(['list'],'readonly'),e=d.objectStore('list').openCursor(),f=[];e.onsuccess=()=>{const b=e.result;b?(f.push(b.value),b.continue()):a(f)},e.onerror=b}).catch(b)});return a}dbOpened(a){this.dbResolve(a.result),this.dbPromise.then((a)=>{a.onerror=(a)=>this.error(a)})}error(a){console.error(a)}createObjectStore(a){const b=a.target,c=b.transaction,d=b.result,e=d.createObjectStore('media',{autoIncrement:!0}),f=d.createObjectStore('list',{keyPath:'id',autoIncrement:!0});0!==a.oldVersion&&(2===a.oldVersion?this.upgrade2to3(e,f,c):d.deleteObjectStore('images'))}upgrade2to3(a,b,c){const d=c.objectStore('images'),e=d.openCursor();e.onerror=(a)=>this.error(a),e.onsuccess=()=>{const d=e.result;if(d){const c=d.value,e={editedId:null,guid:'',id:null,originalId:null,thumbnailId:null,transform:c.transform},f=a.put(c.original);f.onerror=(a)=>this.error(a),f.onsuccess=()=>{if(e.originalId=f.result,c.edited){const d=a.put(c.edited);d.onerror=(a)=>this.error(a),d.onsuccess=()=>{e.editedId=d.result,b.put(e)}}else b.put(e)},d.continue()}else c.db.deleteObjectStore('images')}}}const imageDB=new ImageDB;var ImageState;(function(a){a[a.NOT_LOADED=0]='NOT_LOADED',a[a.LOADED=1]='LOADED',a[a.CHANGED=2]='CHANGED'})(ImageState||(ImageState={}));class ImageRecord{static async fromDatabase(a){const b=await imageDB.retrieveRecord(a);return ImageRecord.fromListRecord(b)}static fromListRecord(a){const b=new ImageRecord;return b.id=a.id,b.guid=a.guid,b.originalState=ImageState.NOT_LOADED,b.editedState=ImageState.NOT_LOADED,b.thumbnailState=ImageState.NOT_LOADED,b.originalId=a.originalId,b.editedId=a.editedId,b.thumbnailId=a.thumbnailId,b.transform=FilterTransform.from(a.transform),b}static async getAll(){const a=await imageDB.all(),b=[];for(const c of a)b.push(ImageRecord.fromListRecord(c));return b}constructor(){this.id=null,this.guid='',this.originalState=ImageState.CHANGED,this.editedState=ImageState.CHANGED,this.thumbnailState=ImageState.CHANGED,this.originalId=null,this.editedId=null,this.thumbnailId=null,this.transform=null,this.originalCache=null,this.editedCache=null,this.thumbnailCache=null}async getOriginal(){return this.originalId&&this.originalState===ImageState.NOT_LOADED&&(this.originalCache=await imageDB.retrieveMedia(this.originalId),this.originalState=ImageState.LOADED),this.originalCache}async getEdited(){return this.editedId&&this.editedState===ImageState.NOT_LOADED&&(this.editedCache=await imageDB.retrieveMedia(this.editedId),this.editedState=ImageState.LOADED),this.editedCache||this.getOriginal()}async getThumbnail(){return this.thumbnailId&&this.thumbnailState===ImageState.NOT_LOADED&&(this.thumbnailCache=await imageDB.retrieveMedia(this.thumbnailId),this.thumbnailState=ImageState.LOADED),this.thumbnailCache||this.getEdited()}setOriginal(a){this.originalCache=a,this.originalState=ImageState.CHANGED}setEdited(a){this.editedCache=a,this.editedState=ImageState.CHANGED}async save(){this.originalState===ImageState.CHANGED&&null!==this.originalCache&&(this.originalId=await imageDB.storeMedia(this.originalCache,this.originalId||void 0)),this.editedState===ImageState.CHANGED&&null!==this.editedCache&&(this.editedId=await imageDB.storeMedia(this.editedCache,this.editedId||void 0)),this.thumbnailState===ImageState.CHANGED&&null!==this.thumbnailCache&&(this.thumbnailId=await imageDB.storeMedia(this.thumbnailCache,this.thumbnailId||void 0));let a={};this.transform&&(a=Object.assign({},this.transform)),this.id=await imageDB.storeRecord({editedId:this.editedId,guid:this.guid,id:this.id,originalId:this.originalId,thumbnailId:this.thumbnailId,transform:a})}}class View{constructor(a){this.viewElement=a}show(){this.viewElement.classList.remove('hidden')}hide(){this.viewElement.classList.add('hidden')}getState(){return this.state||new ViewState}setState(a){this.state=a}}class BrowseView extends View{constructor(){super(document.getElementById('browse-view')),this.captureButton=document.getElementById('browse-capture-button'),this.uploadButton=document.getElementById('browse-upload-button'),this.emptyListElement=document.getElementById('empty-browse-list'),this.listElement=document.getElementById('browse-list'),constants.SUPPORTS_MEDIA_DEVICES||this.captureButton.classList.add('hidden'),this.captureButton.addEventListener('click',()=>this.captureClick()),this.uploadButton.addEventListener('click',()=>this.uploadClick()),this.blobURLs=new Set}async show(){const a=await ImageRecord.getAll();if(0===a.length)this.emptyListElement.classList.remove('hidden'),this.listElement.classList.add('hidden');else{this.emptyListElement.classList.add('hidden'),this.listElement.classList.remove('hidden');for(const b of a){const a=document.createElement('div');a.classList.add('element'),a.addEventListener('click',()=>router.visit(`/edit/${b.id}`));const c=await b.getThumbnail();if(c){const b=URL.createObjectURL(c),d=document.createElement('img');d.src=b,a.appendChild(d),this.blobURLs.add(b)}else;this.listElement.appendChild(a)}}super.show()}hide(){for(super.hide();this.listElement.hasChildNodes();)this.listElement.removeChild(this.listElement.lastChild);for(const a of this.blobURLs)URL.revokeObjectURL(a);this.blobURLs.clear()}captureClick(){router.visit('/capture')}uploadClick(){router.visit('/upload')}}const streamConstraints={video:{deviceId:'',facingMode:['user','environment'],height:{ideal:1080},width:{ideal:1920}}};let supportedConstraints={};constants.SUPPORTS_MEDIA_DEVICES&&(supportedConstraints=navigator.mediaDevices.getSupportedConstraints());class CameraHelper{constructor(){this.stream=null,this.track=null,this.photoCapabilities=null,this.trackConstraints={},this.flash='off'}async getCameras(){let a=[];return constants.SUPPORTS_MEDIA_DEVICES&&(a=await navigator.mediaDevices.enumerateDevices(),a=a.filter((a)=>'videoinput'===a.kind)),a}async takePhoto(a){if(!constants.SUPPORTS_MEDIA_DEVICES)return null;if(constants.SUPPORTS_IMAGE_CAPTURE){const b=a.srcObject;if(!b)return null;const c=b.getVideoTracks()[0],d=new ImageCapture(c),e={};return this.photoCapabilities&&this.photoCapabilities.fillLightMode.includes(this.flash)&&(e.fillLightMode=this.flash),await d.takePhoto(e)}const b=document.createElement('canvas'),c=b.getContext('2d');return b.width=a.videoWidth,b.height=a.videoHeight,c.drawImage(a,0,0),await canvasToBlob(b,constants.IMAGE_TYPE)}stopStream(){if(this.stream)for(const a of this.stream.getVideoTracks())a.stop()}async startStream(a){this.stopStream(),streamConstraints.video.deviceId=a;const b=await navigator.mediaDevices.getUserMedia(streamConstraints);if(this.stream=b,this.track=b.getVideoTracks()[0],constants.SUPPORTS_IMAGE_CAPTURE){const a=new ImageCapture(this.track);this.photoCapabilities=await a.getPhotoCapabilities()}return this.trackConstraints={},b}getPhotoCapabilities(){return this.photoCapabilities?{flash:this.photoCapabilities.fillLightMode,redEyeReduction:'controllable'===this.photoCapabilities.redEyeReduction}:{flash:[],redEyeReduction:!1}}getSettings(){return this.track&&this.track.getSettings?this.track.getSettings():{}}}class CaptureView extends View{constructor(){super(document.getElementById('capture-view')),this.videoElement=this.viewElement.querySelector('video#preview'),this.videoElement2=this.viewElement.querySelector('video#preview2'),this.takePhotoButton=document.getElementById('capture-button'),this.cameraChooseButton=document.getElementById('camera-choose-button'),this.mirrorButton=document.getElementById('capture-mirror-button'),this.flashButton=document.getElementById('capture-flash-button'),this.closeButton=document.getElementById('capture-view-close'),this.videoElement2.classList.add('hidden'),this.cameraHelper=new CameraHelper,this.takePhotoButton.addEventListener('click',()=>this.takePhoto()),this.cameraChooseButton.addEventListener('click',()=>this.toggleCameraChooser()),this.mirrorButton.addEventListener('click',()=>this.toggleMirror()),this.flashButton.addEventListener('click',()=>this.toggleFlash()),this.closeButton.addEventListener('click',()=>this.close()),this.devicesPromise=this.getDevices(),this.currentDevice=null}show(){this.devicesPromise.then((a)=>{this.chooseCamera(a[0])}),super.show()}hide(){this.cameraHelper.stopStream(),this.videoElement.pause(),super.hide()}async getDevices(){const a=await this.cameraHelper.getCameras();return 2>a.length?this.cameraChooseButton.classList.add('hidden'):this.cameraChooseButton.classList.remove('hidden'),a}async takePhoto(){const a=await this.cameraHelper.takePhoto(this.videoElement);a&&this.storeResult(a)}async toggleCameraChooser(){const a=await this.devicesPromise;if(!(2>a.length))if(this.currentDevice){const b=a.indexOf(this.currentDevice);this.chooseCamera(a[(b+1)%a.length])}else this.chooseCamera(a[0])}close(){router.visit(`/browse`)}async chooseCamera(a){this.currentDevice=a,await this.startStream(this.currentDevice.deviceId);const b=this.cameraHelper.getPhotoCapabilities(),c=this.cameraHelper.getSettings();'user'===c.facingMode?this.videoElement.classList.add('mirror'):this.videoElement.classList.remove('mirror'),this.flashButton.classList.remove('flash-flash','flash-off','flash-auto'),this.flashButton.classList.add(`flash-${this.cameraHelper.flash}`),0<b.flash.length?this.flashButton.classList.remove('hidden'):this.flashButton.classList.add('hidden')}async storeResult(a){const b=new ImageRecord;b.setOriginal(a),await b.save(),router.visit(`/edit/${b.id}`)}async startStream(a){const b=await this.cameraHelper.startStream(a);return this.videoElement2.srcObject=b,new Promise((a)=>{this.videoElement2.onloadedmetadata=()=>{const b=this.videoElement;this.videoElement=this.videoElement2,this.videoElement2=b,this.videoElement.play(),this.videoElement.classList.remove('hidden'),this.videoElement2.classList.add('hidden'),this.videoElement2.pause(),a()}})}toggleMirror(){this.videoElement.classList.toggle('mirror')}toggleFlash(){this.flashButton.classList.remove(`flash-${this.cameraHelper.flash}`);const a=this.cameraHelper.getPhotoCapabilities();let b=a.flash.indexOf(this.cameraHelper.flash);b=(b+1)%a.flash.length,this.cameraHelper.flash=a.flash[b],this.flashButton.classList.add(`flash-${this.cameraHelper.flash}`)}}var fragmentShader='\nprecision highp float;\nvarying vec2 texCoords;\nuniform sampler2D textureSampler;\nuniform vec2 sourceSize;\nuniform float saturation;\nuniform float warmth;\nuniform float sharpen;\nuniform float brightness;\nuniform float contrast;\nuniform float blur;\nuniform float vignette;\nuniform float grey;\nvec3 saturationVector = vec3(0.299, 0.587, 0.114);\nvoid main() {\n  vec2 off = sourceSize * blur;\n  vec2 off2 = off * 2.0;\n  vec4 tex00 = texture2D(textureSampler, texCoords + vec2(-off2.x, -off2.y));\n  vec4 tex10 = texture2D(textureSampler, texCoords + vec2(-off.x, -off2.y));\n  vec4 tex20 = texture2D(textureSampler, texCoords + vec2(0.0, -off2.y));\n  vec4 tex30 = texture2D(textureSampler, texCoords + vec2(off.x, -off2.y));\n  vec4 tex40 = texture2D(textureSampler, texCoords + vec2(off2.x, -off2.y));\n  vec4 tex01 = texture2D(textureSampler, texCoords + vec2(-off2.x, -off.y));\n  vec4 tex11 = texture2D(textureSampler, texCoords + vec2(-off.x, -off.y));\n  vec4 tex21 = texture2D(textureSampler, texCoords + vec2(0.0, -off.y));\n  vec4 tex31 = texture2D(textureSampler, texCoords + vec2(off.x, -off.y));\n  vec4 tex41 = texture2D(textureSampler, texCoords + vec2(off2.x, -off.y));\n  vec4 tex02 = texture2D(textureSampler, texCoords + vec2(-off2.x, 0.0));\n  vec4 tex12 = texture2D(textureSampler, texCoords + vec2(-off.x, 0.0));\n  vec4 tex22 = texture2D(textureSampler, texCoords + vec2(0.0, 0.0));\n  vec4 tex32 = texture2D(textureSampler, texCoords + vec2(off.x, 0.0));\n  vec4 tex42 = texture2D(textureSampler, texCoords + vec2(off2.x, 0.0));\n  vec4 tex03 = texture2D(textureSampler, texCoords + vec2(-off2.x, off.y));\n  vec4 tex13 = texture2D(textureSampler, texCoords + vec2(-off.x, off.y));\n  vec4 tex23 = texture2D(textureSampler, texCoords + vec2(0.0, off.y));\n  vec4 tex33 = texture2D(textureSampler, texCoords + vec2(off.x, off.y));\n  vec4 tex43 = texture2D(textureSampler, texCoords + vec2(off2.x, off.y));\n  vec4 tex04 = texture2D(textureSampler, texCoords + vec2(-off2.x, off2.y));\n  vec4 tex14 = texture2D(textureSampler, texCoords + vec2(-off.x, off2.y));\n  vec4 tex24 = texture2D(textureSampler, texCoords + vec2(0.0, off2.y));\n  vec4 tex34 = texture2D(textureSampler, texCoords + vec2(off.x, off2.y));\n  vec4 tex44 = texture2D(textureSampler, texCoords + vec2(off2.x, off2.y));\n  vec4 tex = tex22;\n  vec4 blurred = 1.0 * tex00 + 4.0 * tex10 + 6.0 * tex20 + 4.0 * tex30 + 1.0 * tex40\n               + 4.0 * tex01 + 16.0 * tex11 + 24.0 * tex21 + 16.0 * tex31 + 4.0 * tex41\n               + 6.0 * tex02 + 24.0 * tex12 + 36.0 * tex22 + 24.0 * tex32 + 6.0 * tex42\n               + 4.0 * tex03 + 16.0 * tex13 + 24.0 * tex23 + 16.0 * tex33 + 4.0 * tex43\n               + 1.0 * tex04 + 4.0 * tex14 + 6.0 * tex24 + 4.0 * tex34 + 1.0 * tex44;\n  blurred /= 256.0;\n  tex += (tex - blurred) * sharpen;\n  vec3 desaturated = vec3(dot(saturationVector, tex.rgb));\n  vec3 mixed = mix(desaturated, tex.rgb, saturation);\n  vec4 color = vec4(mixed, tex.a);\n  color.r += warmth;\n  color.b -= warmth;\n  vec3 greyVec = vec3(grey, grey, grey);\n  color.rgb = mix(color.rgb * brightness, mix(greyVec, color.rgb, contrast), 0.5);\n  float ratio = off.x / off.y;\n  float dist = sqrt(pow(texCoords.x - 0.5, 2.0) + pow(ratio * (texCoords.y - 0.5), 2.0));\n  float vigCurve = exp(-pow(dist, 2.0) / (2.0 * pow(-vignette, 2.0)));\n  color.rgb *= vigCurve;\n  gl_FragColor = color;\n}\n';const BASE_VERTEX_SHADER=`
